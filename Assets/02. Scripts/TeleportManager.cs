...
public class TeleportManager : MonoBehaviour
{
    ...
    IEnumerator TeleportSequence(Transform playerTransform)
    {
            ...
            teleportData.targetRoom.SetActive(true);
            playerTransform.transform.position = teleportData.targetRoom.transform.position;


            yield return new WaitForSeconds(0.1f);
            yield return StartCoroutine(FadeManager.Instance.FadeIn(1f));

            if(teleportData.curruntRoom != null)
            {
                teleportData.curruntRoom.SetActive(false);
